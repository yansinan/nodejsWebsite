...
let ControllerPlugin = {
    ...
    getListFields (type = '') {
        ...
        if (type == 'normal') {
            ...
            files = '_id url name alias sImg date discription clickNum roofPlacement type appShowType imageArr videoArr duration simpleComments comments videoImg state dismissReason categories isTop from listHotMusics listLinks listMembers'
        }
    ...
    /**
     * @api {get} /api/artist/getList 查询帖子列表
     * @apiDescription 根据参数获取对应的帖子列表,默认按时间查询，可作为发现栏目列表
     * @apiName /artist/getList
     * @apiGroup Artist
     ...
     *    ]
     *}
     * @apiSampleRequest http://localhost:8080/api/artist/getList
     * @apiVersion 1.0.0
     */
     ...
     async list(ctx, app) {
        ...
        try {
            ...
            let listRes = await ctx.service.artist.find(payload, {
                sort: sortObj,
                query: queryObj,
    ...
    async getTopIndexContents(ctx, app) {
        ...
        try {
            ...
            if (!_.isEmpty(userInfo) && !_.isEmpty(userInfo.watchTags) && userInfo.watchTags.length > 0) {
                ...
                let recContentsNum = await ctx.service.artist.count(tagQuery);
                recContents = await ctx.service.artist.find(payload, {
                    query: tagQuery,
                ...
                if (recContentsNum > current * pageSize) {
                    ...
                    if (leftNormalSize <= pageSize) {
                        ...
                        if (leftNormalSize > 0) {
                            let leftContents = await ctx.service.artist.find({
                                current: 1,
                ...
                let contents = await ctx.service.artist.find(payload, {
                    query: queryObj,
                    files: this.getListFields(),
                    sort: sortObj
                })
    ...
    /**
     * @api {get} /api/artist/getRadom 获取随机文档列表
     * @apiDescription 获取随机文档列表 不分页
     * @apiName /artist/getRadomContents
     * @apiGroup Artist
     * @apiSuccess {json} result
     * @apiSuccessExample {json} Success-Response:
     *{
     ...
     *    ]
     *}
     * @apiSampleRequest http://localhost:8080/api/artist/getRadom
     * @apiVersion 1.0.0
     */
    async getRadom(ctx, app) {
        ...
        try {
            ...
            const totalContents = await ctx.service.artist.count(queryObj);

            randomArticles = await ctx.service.artist.find(_.assign(payload, {
    ...
    async getOne(ctx, app) {
        ...
        try {
            ...
            await ctx.service.artist.inc(ctx, targetId, {
                'clickNum': 1
            })
            let targetContent = await ctx.service.artist.item(ctx, {
                query: queryObj,
            ...
            console.debug("artist.getOne:",targetContent);
    ...
    /**
     * @api {get} /api/artist/getNearbyContent 获取当前文档的上一篇和下一篇
     * @apiDescription 获取随机文档列表 不分页
    ...
    async getNearby(ctx, app) {
        ...
        try {
            ...
            let preContent = await ctx.service.artist.find({
                isPaging: '0',
                pageSize: 1
            }, {
                query: {
            ...
            let nextContent = await ctx.service.artist.find({
                isPaging: '0',
                // pageSize: 1
            }, {
                query: {
    ...
    /**
     * @api {get} /api/artist/getMyFavoriteContents 获取我收藏的帖子列表
     * @apiDescription 获取我收藏的帖子列表 带分页
     * @apiName /artist/getMyFavoriteContents
     * @apiGroup Content
     * @apiParam {string} current 当前页码
     * @apiParam {string} pageSize 每页记录数
    ...
    async getMyFavorite(ctx, app) {
        ...
        try {
            ...
            let favoriteContentsData = await ctx.service.artist.find(payload, {
                query: queryObj,
                searchKeys: ['name'],
                files: this.getListFields()
            })
    ...
    /**
     * @api {post} /api/artist/addOne 创建文档
     * @apiDescription 创建文档
     * @apiName /artist/addOne
     * @apiParam {string}  token 登录时返回的参数鉴权
    ...
    async add(ctx, app) {
        ...
        try {
            ...
            let hadAddContentsNum = await ctx.service.artist.count({
            ...
            let newContent = await ctx.service.artist.create(contentFormObj);
    ...
    /**
     * @api {post} /api/artist/updateOne 更新文档
     * @apiDescription 更新文档
     * @apiName /artist/updateOne
     * @apiParam {string}  token 登录时返回的参数鉴权
    ...
    async update(ctx, app) {
        ...
        try {
            ...
            await ctx.service.artist.update(ctx, fields.id, contentObj);
    ...
    async getCoverList(ctx, app) {
        ...
        try {
            ...
            let coverList = await ctx.helper.reqJsonData(app.config.doracms_api + '/api/artistCover/getList', queryObj);
    ...
    async getOneCover(ctx, app) {
        ...
        try {
            ...
            let queryObj = ctx.query || {};
            let coverObj = await ctx.helper.reqJsonData(app.config.doracms_api + '/api/artistCover/getOne', queryObj);
    ...
    async getRandomImg(ctx, app) {
        ...
        try {
            ...
            const totalContents = await ctx.service.artist.count(queryObj);
            let randomArticles = await ctx.service.artist.find(_.assign(payload, {
                isPaging: '0',
                pageSize,
                skip: Math.floor(totalContents * Math.random())
            }), {

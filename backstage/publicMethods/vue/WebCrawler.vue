<template>
  <div style="text-align: left;">
    <el-tooltip content="输入文章链接" placement="top" effect="light">
...
export default {
    props: {
        ...
        api:{
            type:String,
            default:"/api/content/fetchWX",
        },
        urlKey:{
            type:String,
            default:"mp.weixin.qq.com/s?",
        },
    },
    data() {
        return {
    ...
    methods: {
        ...
        eInputURL(){
            ...
            let strSearch= url.split(that.urlKey)[1];//that.urlKey=="mp.weixin.qq.com/s?",
            // 拆分链接
            if(!strSearch)that.errURL("文章地址不正确");
                ...
                .then(resJson=> {
                    ...
                    if( res && res.status==200 && res.data && res.data.title && res.data.comments){
                        that.$message({
                            message: "找到文章，自动填充："+res.data.title,
                            type: "success"
                        });
                        ...
                        //})
                        // 默认选择第一张图做sImg
                        that.$set(that.objRes,"sImg", res.data.sImg || that.objRes.listSrcImg[0]);
        ...
        submitUpload(){
            ...
            //触发
            if(this.value && this.value.title && this.value.comments) {
                this.$emit('input',this.value);
                this.$emit("confirm",{url:that.$refs.inputURL.value,...this.objRes});
            }
            //关闭
            this.handleClose();
        },

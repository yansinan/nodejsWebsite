// created : 2017.8.21
// author  : Guozhi_Han 
// Email   : Guozhi_Han@126.com

(function rotate(){
     var orientationInit=window.orientation;
     var pd = null;
     function createPd(){
          if(document.getElementById('preventTran') === null){
               var imgData = 'data:image/png;base64,';
               pd = document.createElement('div');
               pd.setAttribute('id','preventTran');
               pd.style.position = 'fixed';
               pd.style.left = '0';
               pd.style.top = '0';
               pd.style.width = 'max(100% , 100vw , calc( 100 * var(--vw)))';
               pd.style.height = 'max(100%, 100vh, calc( 100 * var(--vh)))';
               pd.style.overflow = 'hidden';
               pd.style.backgroundColor = 'var(--color-bg-white)';
               pd.style.textAlign = 'center';
               pd.style.zIndex = '99999';
               pd.style["flex-direction"]="column";
               pd.style["justify-content"]="center";
               document.body.appendChild(pd);
               var img = document.createElement('img');
               img.src = imgData;
               pd.appendChild(img);
               img.style.margin = '2rem auto'
               var br = document.createElement('br');
               var p = document.createElement('p');
               p.style.width = '100%';
               p.style.height = 'auto';
               p.style.fontSize = '1rem';
               p.style.color = '#626262';
               p.style.lineHeight = '1.5rem';
               p.style.textAlign = 'center';
               // p.innerHTML = '为了您的良好体验';
               // p.appendChild(br);
               p.innerHTML += '请竖屏操作';
               pd.appendChild(p);
          }
     }
     if(orientationInit==90||orientationInit==-90){
          if(pd == null && document.getElementById('preventTran') === null) createPd();
          document.getElementById('preventTran').style.display = 'flex';
     }
     window.onorientationchange=function(){
          let orientation="";
          if (window.orientation == 0 || window.orientation == 180)orientation = 'portrait';
          else if (window.orientation == 90 || window.orientation == -90)orientation = 'landscape';
          document.documentElement.classList.value=document.documentElement.classList.value.replace(/(portrait)|(landscape)/g," "+orientation);
          // const $preventTran=document.getElementById('preventTran');
          //if(window.preventTran === null){//之前是竖
          //     if(orientation == 'landscape'){//竖=》横
          //          if(pd == null && window.preventTran == null) createPd();
          //          window.preventTran.style.display = 'flex';                    
          //     }
          //}else{//之前是横
          //     if(orientation == 'portrait'){
          //          window.preventTran.style.display = 'none';
          //          this.dispatchEvent(new CustomEvent("RefreshViewport", {orientation}))
          //     }
          //}
          // 横屏无效
          if(orientation == 'landscape'){
               if(pd == null && window.preventTran == null) createPd();
               window.preventTran.style.display = 'flex';                    
               this.dispatchEvent(new CustomEvent("StopViewport", {orientation}))
          }
          if(orientation == 'portrait'){
               if(window.preventTran) window.preventTran.style.display = 'none';
               this.dispatchEvent(new CustomEvent("RefreshViewport", {orientation}))
          }
     };
 })();